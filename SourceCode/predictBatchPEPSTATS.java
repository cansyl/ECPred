...
public class predictBatchPEPSTATS {
	...
	public static void main(String[] args , Vector<String> ecnums, long time ,String ROOTPATH, String fastaFile) throws IOException, InterruptedException {
		...
		for (int i =0; i< ecnums.size(); i++) {  
		   ...
		   PrintWriter scalefile = new PrintWriter(ROOTPATH+File.separator+"testResult"+File.separator+time+File.separator+time+".sh", "UTF-8");
		   ...
		   scalefile.close();
		   cmd = "chmod +x "+ ROOTPATH+File.separator+"testResult"+File.separator+time+File.separator+time+".sh";
		   ...
		   cmd = ROOTPATH+File.separator+"testResult"+File.separator+time+File.separator+time+".sh";	 
